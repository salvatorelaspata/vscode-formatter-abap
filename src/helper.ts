import { workspace, Range, TextDocument } from 'vscode';
...
export class Helper {
    ...
    /**
     * Logs a message to the console if the abapformatter.logging setting is set to true.
     * 
     * @param message The object to log (anything that console.log can handle).
     */
    public static logDebug(message: any): void {
        if (workspace && workspace.getConfiguration('abapformatter') && workspace.getConfiguration('abapformatter').get('logging', false)) {
            console.log('ABAPFormatter: ' + message);
        }
    }
}